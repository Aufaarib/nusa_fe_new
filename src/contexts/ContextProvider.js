...
export const ContextProvider = ({ children }) => {
  ...
  const [stepsPMB, setStepsPMB] = useState({ 
    ...
    fill_form:{
      status:"",details:{message:""}},test:{status:"",details:{message:""}},payment_education:{status:"",details:{message:""}}
  });
  const [students, setStudents] = useState([{id:"",nama_depan: "",nama_tengah:"",nama_belakang:"",status_anak:"",anak_ke:"",tinggi_badan_anak:"",tinggal_bersama:"",tempat_lahir:"",tanggal_lahir:"",jenis_kelamin:"",golongan_darah:"",hobby_anak:"",berat_badan_anak:"",nomor_kartu_keluarga:"",jarak_rumah_ke_sekolah:"",transportasi_ke_sekolah:"",kelas_saat_mendaftar:"",asal_sekolah:"",sifat_dominan_anak:"",penyakit_pernah_diderita:"",nomor_akta_lahir_anak:""},]);
  ...
  const resendEmailVerification = async (e) => {
    ...
    try {
      ...
      setSuccessMsgSendVerify("Link verifikasi berhasil terkirim. Cek email Anda!");
      setIsLoading(false);
    } catch (err) {
  ...
  const getNotifications = async (markUnread) => {
    ...
    try {
      ...
      const notifUnread = data.data.filter((notification) => notification.read_at === null);
  ...
  const listenBroadcast = async () => {
    try {
      ...
      window.Echo.private('App.Models.User.'+data.id)
  ...
  return (
    <StateContext.Provider value={{daftarUlangAgreement, openForm, setOpenForm, formCheck, setFormCheck, getFormCheck, successMsgSendVerify, setSuccessMsgSendVerify, errMsgSendVerify, setErrMsgSendVerify, resendEmailVerification, paymentAgreement, errStep, setErrStep, stepsPMB, setStepsPMB, getStepsPMBData, documents, setDocuments, getDocumentsData, successMsg, setSuccessMsg, errMsg, setErrMsg, parents, setParents, getParentsData, students, setStudents, getStudentsData, currentPendaftaranId, setCurrentPendaftaranId, isLoading, setIsLoading, currentColor, currentMode, activeMenu, screenSize, setScreenSize, handleClick, isClicked, initialState, setIsClicked, setActiveMenu, setCurrentColor, setCurrentMode, setMode, setColor, themeSettings, setThemeSettings, notifications, getNotifications, notificationNew, setNotificationNew, notificationUnreadLength, setNotificationUnreadLength, listenBroadcast }}>
      {children}
