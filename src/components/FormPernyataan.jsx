...
const FormPernyataan = ({indexMurid}) => {
  ...
  const { isLoading, setIsLoading, errMsg, setErrMsg, parents, setParents, setSuccessMsg, openForm, formCheck, getFormCheck } = useStateContext();
  const [parent, setParent] = useImmer({ id: "", step: 2, ayah: {}, ibu: {}, wali: {}, question: {} });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        const response = await axios.post(PARENTS_URL,
      ...
      <article>
          ...
          <section>
            <div className="flex flex-wrap">
              <label htmlFor="wakaf" className="mt-4 form-label">
                Apakah Ayah dan Bunda bersedia untuk wakaf di Sekolah Adab Insan Mulia<span className="ml-1 text-merah">*</span>
              </label>
              <div className='flex gap-5 w-full mt-2.5 mb-2'>
                <RadioInput id="question" name="wakaf" value="1" label="Ya" onChange={updateParentsRadio} checked={String(parent.question.wakaf)} />
                <RadioInput id="question" name="wakaf" value="0" label="Tidak" onChange={updateParentsRadio} checked={String(parent.question.wakaf)} />
              </div>
            </div>

            <div className={`flex flex-wrap ${parent.question.wakaf == "0" && "hidden"}`}>
              <label htmlFor="nominal_wakaf" className="mt-4 mb-1 form-label">
                Jika Ayah dan Bunda bersedia, berapakah nilai nominal yang ingin diwakafkan<span className="ml-1 text-merah">*</span>
              </label>
            ...
            <div className="flex flex-wrap">
              <label htmlFor="komitmen_infak" className="mt-4 form-label">
                Apakah Ayah dan Bunda bersedia berkomitmen untuk berinfak setiap bulan kepada Sekolah Adab Insan Mulia<span className="ml-1 text-merah">*</span>
              </label>
              <div className='flex gap-5 w-full mt-2.5 mb-2'>
                <RadioInput id="question" name="komitmen_infak" value="1" label="Ya" onChange={updateParentsRadio} checked={String(parent.question.komitmen_infak)} />
                <RadioInput id="question" name="komitmen_infak" value="0" label="Tidak" onChange={updateParentsRadio} checked={String(parent.question.komitmen_infak)} />
              </div>
            </div>

            <div className={`flex flex-wrap ${parent.question.komitmen_infak == "0" && "hidden"}`}>
              <label htmlFor="nominal_infak" className="mt-4 mb-1 form-label">
                Jika Ayah dan Bunda bersedia berinfak setiap bulan, berapakah nilai nominal yang akan diberikan<span className="ml-1 text-merah">*</span>
              </label>
              <DropDownListComponent 
                placeholder=""
                name="question"
        ...
        <section className='flex mt-12'>
          ...
          <div className='flex justify-end w-full'>
            <Link to={"/pmb/form-data-orang-tua"} className="w-auto pl-0 mx-0 bg-transparent shadow-none btn-merah hover:bg-transparent text-merah hover:text-gelap">
              <BsChevronLeft className='text-xl m-0 mr-2 mt-0.5' /> Kembali
            </Link>

            <Link to={"/pmb/berkas-pendaftaran"} className={`${openForm == "form_ortu_pernyataan" && "pointer-events-none text-gray-300"} w-auto pr-0 mx-0 bg-transparent shadow-none btn-merah hover:bg-transparent text-merah hover:text-gelap`}>
